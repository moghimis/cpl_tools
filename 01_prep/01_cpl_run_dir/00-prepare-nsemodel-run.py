...
"""
Dependency:  * Cheeta python template module 
              - pip install cheetah   
              - conda install -c anaconda cheetah=2.4.4
...
#def_files
#adcprep
adcprep   = os.path.join(base_info.application_dir,'ADCIRC/work/adcprep')
tidefac   = os.path.join(base_info.application_dir,'ADCIRC/work/util/tidefac')
adc_inp   = os.path.join(base_info.app_inp_dir,base_info.ocn_inp_dir)
grd_inp   = os.path.join(base_info.app_inp_dir,base_info.grd_inp_dir)

modfile   = os.path.join(base_info.application_dir, base_info.module_file )
nems_exe  = os.path.join(base_info.application_dir,'NEMS/exe/NEMS.x')
...
def get_run_scr():
    ...
    run_scr    = os.path.join(base_info.main_run_dir,base_info.RunName+'_'+base_info.Ver+'_' + curr_time + '.sh' )
    log_file    = os.path.join(base_info.main_run_dir,base_info.RunName+'_'+base_info.Ver+'_' + curr_time + '.log' )
...
def prep_adc(run_dir):
    ...
    os.system('cp -f ' + grd_inp + '/*.*   ' + run_dir)    
    ...
    d15.update(d15_tide)
    # get updated fort.15
    fort15tmp = os.path.join(grd_inp,base_info.fort15_temp)
    ...
    # Treat hotfile
    try:
        base_info.fetch_hot_from
    except NameError:
        base_info.fetch_hot_from = None

    if base_info.fetch_hot_from is not None:
        #print ' > Copy HotStart input file from >', base_info.fetch_hot_from
        
        txt1 = ' > Copy HotStart input file from >  '+ base_info.fetch_hot_from
        logf(txt1,log_file) 
        
        hot_file = os.path.join(base_info.fetch_hot_from,'fort.67.nc')
        os.system('cp -f ' + hot_file +' ' + run_dir)
        hot_file = os.path.join(base_info.fetch_hot_from,'fort.68.nc')        
        os.system('cp -f ' + hot_file +' ' + run_dir)
...
def prep_nems(run_dir):
    ...
    tmpname = os.path.join(base_info.app_inp_dir,'nems',base_info.nems_configure)
    ...
    ##
    tmpname = os.path.join(base_info.app_inp_dir,'nems',base_info.model_configure)
